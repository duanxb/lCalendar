 ...
 * 新增扩展属性和回调函数
 * type [String] 
 * lcalendar: 日期选择区间（最小日期和最大日期合并：2017-01-01,2017-02-01）
...
window.lCalendar = (function() {
    ...
    MobileCalendar.prototype = {
        init: function(params) {
            ...
            this.lcalendar = params.lcalendar;
            this.dayDisabled = params.dayDisabled || [];
            this.weekendDisabled = params.weekendDisabled || false;
            this.onConfirm = params.onConfirm || function() {};
            ...
            if(this.lcalendar){
            	var dateArr = this.lcalendar.split(',');

            	params.minDate = dateArr[0];
            	params.maxDate = dateArr[1];
            }
            if (params.minDate) {
        ...
        bindEvent: function(type) {
            ...
            function popupDate(e) {
                ...
                _self.gearDate.innerHTML = '<div class="date_ctrl slideInUp">' +
                    ...
                    '<div class="current_date_error"></div>' +
            ...
            function finishMobileDate(e) {
                ...
                var selectedDate = (date_yy % passY + _self.minY) + "-" + date_mm + "-" + date_dd;
                if(isChoice(selectedDate) === false){
                    return false;
                }
                _self.trigger.value = selectedDate;
                closeMobileCalendar(e);
                _self.onConfirm(_self.trigger.value);
                document.querySelector("body").style.overflow = "";
            }
            //年月确认
            ...
            //判断当前日期是否可选择
            function isChoice(date){
                if(_self.weekendDisabled){
                    var weekday = new Date(date).getDay();
                    if(weekday == 6 || weekday==0){
                        tipError(date+" 节假日不可选择");
                        return false;
                    }
                }
                if(_self.dayDisabled.length > 0 && _self.dayDisabled.contains(date) === true){
                    tipError(date+"节假日不可选择");
                    return false;
                }
            ...
            //提示错误信息
            function tipError(error){
                var ErrorElement = document.querySelector(".current_date_error");
